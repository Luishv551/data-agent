...
class QueryExecutor:
    ...
    def execute(self, query_intent: Dict[str, Any]) -> QueryResult:
        ...
        if sort_by:
            if sort_by == 'metric' and 'metric_value' in result_df.columns:
                result_df = result_df.sort_values('metric_value', ascending=(sort_order == 'asc'))
            elif sort_by in result_df.columns:
    ...
    def _execute_grouped_query(
        ...
        if metric == 'tpv':
            ...
            grouped = grouped.drop(columns=['amount_transacted'])
            ...
            grouped = grouped.drop(columns=['amount_transacted', 'quantity_transactions'])
            ...
            grouped = grouped.drop(columns=['quantity_transactions'])
            ...
            grouped = grouped.drop(columns=['quantity_of_merchants'])
